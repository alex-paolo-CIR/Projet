...
unique_ptr<Avion> genererNouvelAvion(int& compteurIdAvion, TourControle* tour) {
    ...
    Journaliseur::obtenirInstance().journaliser("Principal", "nouvel avion genere id " + to_string(compteurIdAvion - 1), "INFO");
    return avion;
}
...
unique_ptr<Avion> genererAvionTestCrash(int& compteurIdAvion, TourControle* tour) {
    ...
    // calcul distance jusqua milieu piste
    Position pointApproche = tour->obtenirPointApproche();
    Position seuilPiste = tour->obtenirSeuilPiste();
    Position milieuPiste(400.0f, 300.0f);
  ...
  float distanceApprocheVersSeuil = pointApproche.distanceVers(seuilPiste);
    ...
    Journaliseur::obtenirInstance().journaliser("Principal", ss.str(), "WARN");
...
float echelleY = 600.0f / tailleTexture.y;
        ...
        // scenario charge lourde 10 avions au demarrage espaces verticalement
        vector<unique_ptr<Avion>> avions;
        int compteurIdAvion = 1;
 
Journaliseur::obtenirInstance().journaliser("Principal", "scenario charge lourde 10 avions", "INFO");
     ...
     // position x variee
      ...
      Journaliseur::obtenirInstance().journaliser("Principal", ss.str(), "INFO");
   }

        Journaliseur::obtenirInstance().journaliser("Principal", "simulation demarree 10 avions espaces", "INFO");
  ...
  fenetre.draw(*spriteFond);
     ...
     // dessiner piste
            RectangleShape piste(Vector2f(200, 20));
         piste.setPosition(Vector2f(300, 290));
 piste.setFillColor(pisteFermee ? Color(150, 0, 0) : Color(80, 80, 80));
     piste.setOutlineColor(Color::White);
       piste.setOutlineThickness(2);
            fenetre.draw(piste);
  
        // marquages piste
if (!pisteFermee) {
 for (int i = 0; i < 5; ++i) {
             RectangleShape marquage(Vector2f(20, 5));
           marquage.setPosition(Vector2f(320 + i * 40, 297.5f));
         marquage.setFillColor(Color::Yellow);
   fenetre.draw(marquage);
       }
    } else {
        // croix rouge piste fermee
              RectangleShape croix1(Vector2f(200, 5));
                croix1.setPosition(Vector2f(300, 297.5f));
        croix1.setFillColor(Color::Red);
    croix1.setRotation(degrees(45));
     croix1.setOrigin(Vector2f(100, 2.5f));
              croix1.setPosition(Vector2f(400, 300));
             fenetre.draw(croix1);
         
  RectangleShape croix2(Vector2f(200, 5));
   croix2.setPosition(Vector2f(300, 297.5f));
                croix2.setFillColor(Color::Red);
  croix2.setRotation(degrees(-45));
       croix2.setOrigin(Vector2f(100, 2.5f));
    croix2.setPosition(Vector2f(400, 300));
    fenetre.draw(croix2);
 }
...
placeParking.setPosition(Vector2f(posParking.x - 15, posParking.y - 15));
  ...
  texteId.setString(to_string(avion->obtenirId()));
    ...
    // supprimer avions inactifs mais pas crashes
            avions.erase(
    remove_if(avions.begin(), avions.end(),
          [](const unique_ptr<Avion>& a) { 
            return !a->estActif() && a->obtenirEtat() != EtatAvion::ECRASE; 
      }),
          avions.end()
         );
 ...
 // dessiner statistiques
   ...
   Journaliseur::obtenirInstance().journaliser("Principal", "arret avions", "INFO");
     for (auto& avion : avions) {
            avion->arreter();
        }
    ...
    for (auto& avion : avions) {
     avion->rejoindre();
        ...
        Journaliseur::obtenirInstance().journaliser("Principal", "simulation terminee", "INFO");
        Journaliseur::obtenirInstance().fermer();
        
    } catch (const exception& e) {
cerr << "erreur " << e.what() << endl;
        Journaliseur::obtenirInstance().journaliser("Principal", string("exception ") + e.what(), "ERROR");
        Journaliseur::obtenirInstance().fermer();
  return EXIT_FAILURE;
    }
